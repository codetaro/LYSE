...
handle_event({complete, Regex, Ref, Count}, State, Data = #data{regex = Re, refs = Refs}) ->
  {Regex, OldCount} = lists:keyfind(Regex, 1, Re),                   % find the regex that just completed in the Re list
  NewRe = lists:keyreplace(Regex, 1, Re, {Regex, OldCount + Count}), % update the value with the new count
  NewData = Data#data{regex = NewRe, refs = Refs--[Ref]},            % update the data record while removing the Ref
  case State of                                                      % send to the next state
    dispatching ->
